...
To drive towards an end goal as we learn about `ggplot`, lets create a visualization to depect the annual hit totals for players with the top 20 total career hits.
...
Bar plots often suffere from difficult to read x axes.  We can fix that with a coordinate flip.
...
Lets start with our origional data:
...
scatterdat <- 
  ...
  summarise_at(.vars = vars(g:gidp),.funs = list(sum))
...
`ggplot` offers the ability to 'facet' plots by a variable.  This can help show contrats between different factors.
